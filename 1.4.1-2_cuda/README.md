...
Uses PyTorch 2.x and Piper 1.4.1-2 (077c783a28a8eb31c269bf2fc79e1f834d265809).
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    ...
    -it harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.4.1-2_cuda
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/piper:1.4.1-2_cuda
  ```

### Build local image

* Build the image from Docker file (from within /path_to/piper/1.4.1-2_cuda)
...
```bash
docker build -t piper:1.4.1-2_cuda .
```
...
* Tag
  ...
  docker tag \
    piper:1.4.1-2_cuda \
    harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.4.1-2_cuda
  ```
  
* Push

  ```bash
  docker push harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.4.1-2_cuda
  ```
...
* Tag
  ...
  docker tag \
    piper:1.4.1-2_cuda \
    waikatodatamining/piper:1.4.1-2_cuda
  ```
  
* Push

  ```bash
  docker push waikatodatamining/piper:1.4.1-2_cuda
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
