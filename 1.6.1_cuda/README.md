...
Uses PyTorch 2.x and Piper 1.6.1 (5a9a99d528fb8d3d916f18ea453276e00bdae320).
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    ...
    -it harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.6.1_cuda
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/piper:1.6.1_cuda
  ```

### Build local image

* Build the image from Docker file (from within /path_to/piper/1.6.1_cuda)
...
```bash
docker build -t piper:1.6.1_cuda .
```
...
* Tag
  ...
  docker tag \
    piper:1.6.1_cuda \
    harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.6.1_cuda
  ```
  
* Push

  ```bash
  docker push harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.6.1_cuda
  ```
...
* Tag
  ...
  docker tag \
    piper:1.6.1_cuda \
    waikatodatamining/piper:1.6.1_cuda
  ```
  
* Push

  ```bash
  docker push waikatodatamining/piper:1.6.1_cuda
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
