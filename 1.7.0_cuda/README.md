...
Uses PyTorch 2.x and Piper 1.7.0 (7b8e8f7197a480047677715f00d3d78903b55a2a).
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/piper:1.7.0_cuda
  ```

### Build local image

* Build the image from Docker file (from within /path_to/piper/1.7.0_cuda)
...
```bash
docker build -t piper:1.7.0_cuda .
```
...
* Tag
  ...
  docker tag \
    piper:1.7.0_cuda \
    harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.7.0_cuda
  ```
  
* Push

  ```bash
  docker push harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.7.0_cuda
  ```
...
* Tag
  ...
  docker tag \
    piper:1.7.0_cuda \
    waikatodatamining/piper:1.7.0_cuda
  ```
  
* Push

  ```bash
  docker push waikatodatamining/piper:1.7.0_cuda
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
