...
def tts_args(speaker_id: int = None, length_scale: float = None, noise_scale: float = None,
             noise_w: float = None) -> SynthesisConfig:
    """
...
def tts_to_file(voice: PiperVoice, synthesize_args: SynthesisConfig, text: str, output: str) -> str:
    """
    ...
    :type synthesize_args: SynthesisConfig
    ...
    with wave.open(output, "wb") as fp:
        voice.synthesize_wav(text, fp, syn_config=synthesize_args)
    return output


def tts_to_data(voice: PiperVoice, synthesize_args: SynthesisConfig, text: str) -> bytes:
    """
