# Piper

Uses [Piper](https://github.com/OHF-Voice/piper1-gpl). 

Uses PyTorch 2.x and Piper 1.8.0 (639388b6317fc4731e91d53da42aea68fd4166ff).

## Quick start

### Inhouse registry

* Log into registry using *public* credentials:

  ```bash
  docker login -u public -p public public.aml-repo.cms.waikato.ac.nz:443 
  ```

* Pull and run image (adjust volume mappings `-v`):

  ```bash
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.8.0_cuda
  ```

### Docker hub

* Pull and run image (adjust volume mappings `-v`):

  ```bash
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/piper:1.8.0_cuda
  ```

### Build local image

* Build the image from Docker file (from within /path_to/piper/1.8.0_cuda)

  ```bash
  docker build -t piper .
  ```
  
* Run the container

  ```bash
  docker run --shm-size 8G --net=host -v /local/dir:/container/dir -it piper
  ```
  `/local/dir:/container/dir` maps a local disk directory into a directory inside the container


## Publish images

### Build

```bash
docker build -t piper:1.8.0_cuda .
```

### Inhouse registry  

* Tag

  ```bash
  docker tag \
    piper:1.8.0_cuda \
    harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.8.0_cuda
  ```
  
* Push

  ```bash
  docker push harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.8.0_cuda
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
  
  ```bash
  docker login public-push.aml-repo.cms.waikato.ac.nz:443
  ```

### Docker hub  

* Tag

  ```bash
  docker tag \
    piper:1.8.0_cuda \
    waikatodatamining/piper:1.8.0_cuda
  ```
  
* Push

  ```bash
  docker push waikatodatamining/piper:1.8.0_cuda
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
  
  ```bash
  docker login
  ``` 


### Requirements

```bash
docker run --rm --pull=always -u $(id -u):$(id -g) -e USER=$USER \
  -v `pwd`:/workspace \
  --entrypoint /opt/piper/bin/pip \
  -it harbor.cms.waikato.ac.nz/public/puwhakahua/piper:1.8.0_cuda \
  --no-cache-dir freeze > requirements.txt
```


## Permissions

When running the docker container as regular use, you will want to set the correct
user and group on the files generated by the container (aka the user:group launching
the container):

```bash
docker run -u $(id -u):$(id -g) -e USER=$USER ...
```

## Caching

piper uses numba and the compiled files need to be cached in a directory with
write access. Therefore, it is recommended to create a `tmp` directory
and map it into the container:

```bash
-v `pwd`/tmp:/tmp \
```


## Scripts

The following additional scripts are available:

* `piper_train` - for training a model (calls `python -m piper_train`)
* `piper_export_onnx` - for exporting a checkpoint to ONNX (calls `python -m piper_train.export_onnx`)
* `piper` - for applying a model t (calls `python -m piper`)
* `piper_tts_poll` - for batch processing text files and generating WAV files (calls `/opt/piper-extra/tts_poll.py`)
* `piper_tts_redis` - uses Redis for receiving text and sending back generated WAV data (calls `/opt/piper-extra/tts_redis.py`)


## Troubleshooting

* `OSError: [Errno 16] Device or resource busy: '/tmp/...'`

  Instead of training the model off a NFS network share, use a local disk

